...
/*
 * Servers configuration, add additional servers by environment variables
 */
if( getenv('db_host') &&
    getenv('db_name')
)
{
    ...
    $cfg['Servers'][$i]['verbose'] = getenv('db_name') . '@' . getenv('db_host');
    $cfg['Servers'][$i]['auth_type'] = 'cookie';
    /* Server parameters */
...
foreach ( $aEnvUrlNames as $sUrl)
{
    if( getenv( $sUrl))
    {
        $aUrl = parse_url( getenv( $sUrl));

        $i += $incremnt;

        $cfg['Servers'][$i]['verbose'] = basename( $aUrl['path']) . '@' . $aUrl['host'];
        $cfg['Servers'][$i]['auth_type'] = 'cookie';
        /* Server parameters */
        $cfg['Servers'][$i]['host'] = $aUrl['host'];
        $cfg['Servers'][$i]['port'] = $aUrl['port'];
        $cfg['Servers'][$i]['connect_type'] = 'tcp';
        $cfg['Servers'][$i]['compress'] = false;
        $cfg['Servers'][$i]['extension'] = 'mysqli';
        $cfg['Servers'][$i]['AllowNoPassword'] = false;
